...
const API_URL = () =>
  `${(global.APIs?.delirius?.url || 'https://api.delirius.online').replace(/\/$/, '')}/nsfw/boobs`
...
let handler = async (m, { conn }) => {
  try {
    ...
    console.error('Error en comando tetas:', error)
    m.reply('*[❗] Ocurrió un error al obtener la imagen. Por favor, inténtalo de nuevo más tarde.*')
  }
}

handler.help = ['#tetas', '#tch', '#ts']
handler.tags = ['nsfw']
handler.command = ['tetas', 'tch', 'ts']
