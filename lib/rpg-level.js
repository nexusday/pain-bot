...
export function levelUpReward(completedLevel) {
  const lv = Math.max(1, Math.floor(Number(completedLevel) || 1))
  if (lv === 1) return 1000
  if (lv === 2) return 1600
  return Math.floor(1600 * Math.pow(1.28, lv - 2) + (lv - 2) * 180)
}

export function nextLevelReward(currentLevel) {
  return levelUpReward(Math.max(1, Math.floor(Number(currentLevel) || 1)))
}
...
function applyLevelUps(user) {
  ...
  while (guard++ < 5000) {
    ...
    const completed = user.level
    const prize = levelUpReward(completed)
    user.exp -= need
    user.level += 1
    user.coins = (Number(user.coins) || 0) + prize
    rewardTotal += prize
    gained += 1
  }

  return { gained, rewardTotal }
}
...
export function awardCommandProgress(user, { isSticker = false } = {}) {
  ...
  const { gained: levelsGained, rewardTotal } = applyLevelUps(user)
  const need = xpRequiredForLevel(user.level)
  ...
  return {
    ...
    before,
    reward: rewardTotal,
    nextReward: nextLevelReward(user.level),
...
export function formatLevelUpMessage(rpg) {
  const moneda = rpg.moneda || global.moneda || 'USD'
  const reward = formatNumber(rpg.reward || 0)
  const next = formatNumber(rpg.nextReward || 0)
  return [
    `☾ *¡Subiste de nivel!*`,
    ``,
