...
export function makeWASocket(opcionesConexion, opciones = {}) {
  ...
  // Reply verificado: pushName + foto de perfil (en vez del texto /menu)
  const enviarMensajeOriginal = conn.sendMessage.bind(conn);
  conn.sendMessage = async (jid, content, options = {}) => {
    const opts = await aplicarQuotedVerificado(conn, options || {});
    return await enviarMensajeOriginal(jid, content, opts);
  };
              ...
              sender: {
                get() {
                  try {
                    ...
                    return decodificarJidSeguro(participanteCrudo, self.conn) || participanteCrudo;
                  } catch (e) {
