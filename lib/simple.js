    ...
    getFile: {
      ...
      async value(PATH, saveToFile = false) {
        ...
        if (data && saveToFile && !filename) {
          ...
          filename = path.join(PROJECT_TMP, `${Date.now()}.${type.ext.replace(/^\./, '')}`);
        ...
        return {
          ...
          data,
          deleteFile() {
            if (!isTemp || !filename) return Promise.resolve();
    ...
    sendListLia: {
      async value(jid, title, text, buttonText, listSections, quoted, options = {}) {
        ...
        return sendViaLia(conn, jid, {
          ...
          buttonText,
          title,
          sections: normalizeLiaListSections(listSections),
        }, { quoted, ...options })
      },
      enumerable: true,
    },
      ...
      async value(m) {
        ...
        switch (m.messageStubType) {
          ...
          case WAMessageStubType.GROUP_CHANGE_INVITE_LINK:
            if (stubParams[0]) emitGroupUpdate({ revoke: stubParams[0] });
            break;
          case WAMessageStubType.GROUP_CHANGE_ICON:
            if (stubParams[0]) emitGroupUpdate({ icon: stubParams[0] });
            break;
          default: {
            console.log({
      ...
      get() {
        try {
          ...
          if (mtype === "listResponseMessage") {
            const rowId = msg?.singleSelectReply?.selectedRowId;
          ...
          if (mtype === "templateButtonReplyMessage") {
            const tplId = msg?.selectedId;
            if (tplId) return tplId;
          }
