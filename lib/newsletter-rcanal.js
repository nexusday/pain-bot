...
export function getChannelFollowEntries() {
  ...
  const logs = readLogsSubbotsConfig()
  if (logs.jid) entries.add(logs.jid)
  else if (logs.invite) entries.add(logs.invite)
  else if (global.logssubbots) {
    if (typeof global.logssubbots === 'string' && global.logssubbots.trim()) {
      entries.add(global.logssubbots.trim())
    } else if (typeof global.logssubbots === 'object') {
      const raw = global.logssubbots.jid || global.logssubbots.id || global.logssubbots.link || global.logssubbots.url || global.logssubbots.invite || ''
      if (String(raw).trim()) entries.add(String(raw).trim())
    }
  }

  if (global.idlogssubbots) entries.add(global.idlogssubbots)
