...
export function construirTextoRafaga(items) {
  ...
  return String(ultimo?.text || '').trim()
}
...
export function crearPuertaChat(opciones = {}) {
  ...
  function claveUsuario(chatId, item) {
    const chat = normalizarIdChat(chatId) || String(chatId || '')
    const crudo =
      item?.m?.sender ||
      item?.sender ||
      item?.m?.participant ||
      item?.m?.key?.participant ||
      'unknown'
    let sender = String(crudo)
    try {
      if (typeof sender.decodeJid === 'function') sender = sender.decodeJid()
    } catch {}
    sender = sender.replace(/:\d+@/g, '@').split(':')[0]
    return `${chat}::${sender}`
  }

  function obtenerEstado(clave) {
  ...
  function armarTemporizador(clave) {
  ...
  function programar(chatId, item, ejecutarTrabajo) {
    const clave = claveUsuario(chatId, item)
    const estado = obtenerEstado(clave)
    const ahora = Date.now()
  ...
  async function vaciar(clave) {
    ...
    const textoUltimo = construirTextoRafaga(items)
    ...
    try {
      actuado = await Promise.race([
        ...
        console.error(`${label} [${clave}]:`, err?.message || err)
        return false
      })
      ...
      if (estado.lote.length) armarTemporizador(clave)
    }
