...
const puertaIlegal = crearPuertaChat({ label: 'Modo Ilegal' })

export async function manejarModoIlegal(m, conn) {
  if (!m.isGroup || !estaModoActivo('modoIlegal', m.chat) || m.fromMe) return false
  if (!esMensajeTextoPlano(m)) return false
  if (estaModoActivo('modoHuman', m.chat) || estaModoActivo('modoSad', m.chat) || estaModoActivo('modoPsico', m.chat) || estaModoActivo('modoCustom', m.chat)) return false
  if (estaModoActivo('modoIA', m.chat) || estaModoActivo('modoHot', m.chat)) return false

  const texto = m.text.trim()
  if (texto.length < 4) return false

  return puertaIlegal.schedule(
    normalizarIdChat(m.chat),
    { m, conn, text: texto },
    (trabajo) => ejecutarModoIlegal(trabajo)
  )
}

async function ejecutarModoIlegal(trabajo) {
  const m = trabajo.m
  const conn = trabajo.conn
  const texto = trabajo.text
  const estaObsoleto = trabajo.isStale || (() => false)
  if (!m || !conn || !estaModoActivo('modoIlegal', m.chat)) return false
  ...
  try {
    ...
    const respuesta = await callGeminiIlegalAPI(texto, nombreUsuario, nombreGrupo, m.chat)
...
export { manejarModoIlegal as handleModoIlegal }
