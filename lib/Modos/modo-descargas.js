...
async function downloadTikTok(url) {
  // Import dinámico evita ciclos y reutiliza el fix de tiktok-2
  const { downloadTikTok: dl } = await import('../plugins/tiktok-2.js')
  try {
    return await dl(url)
  } catch (e) {
    console.error('modo-descargas tiktok:', e?.message || e)
    return null
  }
}
