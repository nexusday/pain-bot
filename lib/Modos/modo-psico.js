...
async function simulatePsicoTyping(conn, chat, message = '', isStale) {
  const ms = Math.min(2200, Math.max(900, 800 + String(message).length * 28 + Math.floor(Math.random() * 400)))
  ...
  while (remaining > 0) {
    ...
    const wait = Math.min(1000, remaining)
    await delay(wait)
    remaining -= wait
    if (remaining > 0) {
      await conn.sendPresenceUpdate('composing', chat).catch(() => {})
...
export async function handleModoPsico(m, conn) {
  if (!m.isGroup || !isModoPsicoActive(m.chat) || m.fromMe) return false
  ...
  return psicoGate.schedule(
    ...
    { m, conn, text },
    (job) => executeModoPsico(job)
  )
}

async function executeModoPsico(job) {
  const m = job.m
  const conn = job.conn
  const text = job.text
  const isStale = job.isStale || (() => false)
  if (!m || !conn || !isModoPsicoActive(m.chat)) return false
  ...
  try {
    ...
    if (decision.action === 'reply' && decision.message) {
      const ok = await simulatePsicoTyping(conn, m.chat, decision.message, isStale)
