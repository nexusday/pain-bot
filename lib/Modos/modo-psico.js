...
async function simularEscrituraPsico(conn, chat, mensaje = '', estaObsoleto) {
  const ms = Math.min(1100, Math.max(300, 250 + String(mensaje).length * 16 + Math.floor(Math.random() * 200)))
  ...
  while (restante > 0) {
    ...
    const espera = Math.min(450, restante)
...
export async function manejarModoPsico(m, conn) {
  ...
  if (
    ...
    estaModoActivo('modoCustom', m.chat)
  ) {
    return false
  }
...
async function ejecutarModoPsico(trabajo) {
  ...
  try {
    ...
    if (decision.action === 'react' && decision.emoji) {
      await retraso(160 + Math.floor(Math.random() * 280))
      if (estaObsoleto()) return false
