...
export const DUPLICATE_CHANCE = 0.22
...
export async function fetchRandomCharacter(opts = {}) {
  ...
  const ownedStacks = Array.isArray(opts.ownedStacks) ? opts.ownedStacks : []
  await ensureCatalogWarm(60)

  const store = loadCatalog()

  // Soft pity de repetición: ~22% si el usuario ya tiene colección
  const dupChance = Number.isFinite(opts.duplicateChance)
    ? Math.min(0.85, Math.max(0, opts.duplicateChance))
    : DUPLICATE_CHANCE

  if (ownedStacks.length > 0 && Math.random() < dupChance) {
    const ownedPick = pickOwnedDuplicate(ownedStacks, store)
    if (ownedPick && !excludeIds.has(String(ownedPick.id))) {
      return toRuntimeCharacter(ownedPick)
    }
  }
