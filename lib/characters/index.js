...
export async function giftHaremCharacter(m, conn, usedPrefix = '.', args = []) {
  ...
  const slotToken = (args || [])
    .map(a => String(a || '').replace(/[^\d]/g, ''))
    .find(a => /^\d+$/.test(a))

  if (!targetJid || !slotToken) {
    await conn.sendMessage(m.chat, {
      text: `[❗] Uso: *${usedPrefix}darhr @usuario <n°>*\n> Ejemplo: *${usedPrefix}darhr @user 1*\n> El número es el orden de *${usedPrefix}harem*`,
      contextInfo: { ...(global.rcanal?.contextInfo || {}) },
    }, { quoted: m })
    return
  }

  if (String(targetJid) === String(m.sender)) {
    await conn.sendMessage(m.chat, {
  ...
  const idx = findCharacterIndex(fromUser, slotToken)
  if (idx < 0) {
    await conn.sendMessage(m.chat, {
      text: `[❗] No encontré ese slot en tu harem.\n> Revisa *${usedPrefix}harem*`,
      contextInfo: { ...(global.rcanal?.contextInfo || {}) },
    }, { quoted: m })
    return
  }
  ...
  if (!result.ok) {
    const map = {
      missing: 'No encontré ese personaje.',
      listed: `*${result.name}* está en venta. Cancela primero con *${usedPrefix}cancelarg <orden>* o usa otra copia libre.`,
      empty: 'No tienes unidades libres de ese personaje.',
    }
    await conn.sendMessage(m.chat, {
      text: `[❗] ${map[result.reason] || 'No se pudo regalar.'}`,
      contextInfo: { ...(global.rcanal?.contextInfo || {}) },
    }, { quoted: m })
    return
  }
