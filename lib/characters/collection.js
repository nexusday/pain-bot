...
function resolveCombat(id, price, catalog) {
  if (catalog?.maxHp && catalog?.power) {
    return {
      maxHp: Number(catalog.maxHp) || 0,
      hp: Number(catalog.hp || catalog.maxHp) || 0,
      power: Number(catalog.power) || 0,
    }
  }
  return combatStatsForPrice(price, id)
}
...
function normalizeOwnedCharacter(c = {}) {
  ...
  const combat = resolveCombat(id, locked.price, catalog || locked)
  return {
    id,
...
export function giftCharacterFromSlot(fromUser, toUser, index) {
  ensureCharUser(fromUser)
  ensureCharUser(toUser)
  const i = Number(index)
  if (!Number.isInteger(i) || i < 0 || i >= fromUser.characters.length) {
    return { ok: false, reason: 'missing' }
  }
  const stack = fromUser.characters[i]
  if (availableQty(stack) < 1) {
    return {
      ok: false,
      reason: (Number(stack.listed) || 0) > 0 ? 'listed' : 'empty',
      name: stack.name,
      listed: stack.listed || 0,
      qty: stack.qty,
    }
  ...
  const piece = takeCharacterFromSlot(fromUser, i, 1)
  if (!piece) return { ok: false, reason: 'missing' }

  addCharacter(toUser, piece, 1)
  return { ok: true, character: piece }
}
