...
async function getGroupParticipants(conn, m) {
  const cached = conn?.chats?.[m.chat]?.metadata?.participants
...
async function deleteWithAdminBot(m, conn, participants) {
  const deleteKey = buildDeleteKey(m)
  const adminConn = findAdminBotConn(participants, conn)

  const toTry = []
  const seen = new Set()
  const add = (c) => {
    const id = c?.user?.jid || c?.user?.id
    if (!c?.sendMessage || seen.has(id)) return
    seen.add(id)
    toTry.push(c)
  }
  add(adminConn)
  ...
  for (const c of toTry) {
    try {
      await c.sendMessage(m.chat, { delete: deleteKey })
      return true
    } catch {}
    try {
...
export async function handleAntiPalabra(m, conn, isAdmin, rcanal, isOwner = false) {
  try {
    ...
    const participants = await getGroupParticipants(conn, m)
    const { isAdmin: senderIsAdmin } = getParticipantAdminFlags(participants, m, conn)
    if (senderIsAdmin || isOwner) return false
    ...
    const deleted = await deleteWithAdminBot(m, conn, participants)
    const adminConn = findAdminBotConn(participants, conn) || conn
    const ctx = { contextInfo: { ...rcanal.contextInfo, mentionedJid: [m.sender] } }

    if (action === 'kick') {
      if (deleted) {
        await adminConn.sendMessage(m.chat, {
          text: `@${m.sender.split('@')[0]} usó una palabra prohibida y será expulsado.`,
          ...ctx,
        }).catch(() => {})
      }
      await adminConn.groupParticipantsUpdate(m.chat, [m.sender], 'remove').catch(() => {})
      if (!deleted) {
