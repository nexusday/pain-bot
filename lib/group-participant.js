...
export function jidsOverlap(aList, bList) {
...
export function resolveTargetJids(who, participants, conn) {
  const base = senderJidCandidates(null, conn, [who].filter(Boolean))
  const p = findGroupParticipant(participants, who, conn)
  const fromParticipant = participantJids(p, conn)
  return [...new Set([...base, ...fromParticipant].filter(Boolean))]
}


export function isSenderMuted(mutedList, m, conn, participants) {
  if (!Array.isArray(mutedList) || !mutedList.length) return false

  const senderIds = senderJidCandidates(m, conn)
  if (jidsOverlap(senderIds, mutedList)) return true

 
  const p = findGroupParticipant(participants, m, conn)
  if (p && jidsOverlap(participantJids(p, conn), mutedList)) return true

  return false
}


export function removeMutedMatches(mutedList, targetJids) {
  if (!Array.isArray(mutedList)) return []
  const targets = Array.isArray(targetJids) ? targetJids : [targetJids]
  return mutedList.filter(j => !jidsOverlap([j], targets))
}
...
export function isBotParticipantAdmin(participants, conn) {
  const bot = findBotParticipant(participants, conn)
  return !!(bot?.admin)
}

export function collectBotConnections(preferredConn) {
  const list = []
  const seen = new Set()
  const add = (c) => {
  ...
  add(preferredConn)
  add(global.conn)
  if (Array.isArray(global.conns)) global.conns.forEach(add)
  return list
}

/** Devuelve la conexion (bot principal o sub-bot) que sea admin en el grupo. */
export function findAdminBotConn(participants, preferredConn) {
  for (const c of collectBotConnections(preferredConn)) {
    if (isBotParticipantAdmin(participants, c)) return c
  }
  return null
}
