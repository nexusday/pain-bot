...
(function(){
  ...
  async function apiPost(path, body){
    ...
    const res=await fetch(API_BASE+path,{
      ...
      body:JSON.stringify(Object.assign({sessionId:SESSION_ID,token:TOKEN},body||{}))
    });
    let data=null;
  ...
  async function submitScore(pts){
    ...
    try{
      const r=await apiPost('/api/snake/score',{score:pts});
      const d=r&&r.data||{};
      if(d.ok){
        ...
        setSave('ok', d.newRecord?'¡Nuevo récord!':'Guardado en el top',
          pts+' pts · récord '+d.best+(d.total!=null?' · total '+d.total:''));
      } else {
      ...
      setSave('err','Sin conexión','No alcanzó el servidor del bot');
    }finally{
      saving=false;
    }
  }
