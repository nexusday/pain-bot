...
const I = {
  ...
  check: `<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round"><path d="M20 6 9 17l-5-5"/></svg>`
}
 ...
 * @param {{base?:string,sessionId?:string,token?:string}|null} opts.api
 */
export function htmlSnake(opts = {}) {
  ...
  const api = opts.api && typeof opts.api === 'object' ? opts.api : null
  const apiBase = escUrl(api?.base || '')
  const sessionId = escUrl(api?.sessionId || '').slice(0, 64)
  const token = escUrl(api?.token || '').slice(0, 96)
  const hasApi = Boolean(apiBase && sessionId && token)
...
.save{margin-top:4px;width:100%;max-width:250px;background:#0b1220;border:1px dashed rgba(34,197,94,.35);border-radius:12px;padding:8px 10px}
.save.err{border-color:rgba(239,68,68,.4)}
.save.ok{border-color:rgba(34,197,94,.55)}
.save label{display:block;font-size:10px;color:var(--muted);margin-bottom:4px}
.save code{display:block;font-size:12px;font-weight:700;color:var(--accent);font-family:ui-monospace,SFMono-Regular,Menlo,monospace;letter-spacing:.2px;line-height:1.35}
.save.err code{color:#fca5a5}
...
<div class="app">
  ...
  <div class="stage">
    ...
    <div class="panel start" id="start">
      ...
      <h2>Tu partida</h2>
      <p>Solo tú juegas aquí. Al terminar se guarda solo en el top.</p>
    ...
    <div class="panel over hidden" id="over">
      ...
      <div class="save" id="saveBox">
        <label id="saveLabel">Puntuación</label>
        <code id="saveStatus">…</code>
      </div>
  ...
  <div class="solo">Auto-guardado · Top: ${prefix}topsnake</div>
</div>
<script>
(function(){
  ...
  const N=16, CELL=canvas.width/N, TICK=120;
  let snake, dir, nextDir, food, score, alive, last=0, raf=0, bestLocal=BEST0, saving=false;

  function setSave(state, label, text){
    saveBox.classList.remove('ok','err');
    if(state) saveBox.classList.add(state);
  ...
  async function apiPost(path, body){
    ...
    const payload=Object.assign({sessionId:SESSION_ID,token:TOKEN},body||{});
    const res=await fetch(API_BASE+path,{
      method:'POST',
      headers:{
        'Content-Type':'application/json',
        'Authorization':'Bearer '+TOKEN,
        'X-Snake-Session':SESSION_ID
      },
      body:JSON.stringify(payload)
    });
    let data=null;
    try{ data=await res.json(); }catch(e){ data={ok:false,error:'json'}; }
    return {status:res.status,data:data||{}};
  }

  function apiGetScore(pts){
    const q='sessionId='+encodeURIComponent(SESSION_ID)+
      '&token='+encodeURIComponent(TOKEN)+
      '&score='+encodeURIComponent(String(pts));
    return fetch(API_BASE+'/api/snake/score?'+q,{method:'GET',cache:'no-store'})
      .then(async res=>{
        let data=null;
        try{ data=await res.json(); }catch(e){ data={ok:false,error:'json'}; }
        return {status:res.status,data:data||{}};
  ...
  function apiBeaconScore(pts){
    return new Promise(resolve=>{
      const q='sessionId='+encodeURIComponent(SESSION_ID)+
        '&token='+encodeURIComponent(TOKEN)+
        '&score='+encodeURIComponent(String(pts));
      const url=API_BASE+'/api/snake/score?'+q;
      let done=false;
      const finish=ok=>{ if(done) return; done=true; resolve(ok); };
      try{
        if(navigator.sendBeacon&&navigator.sendBeacon(url)){
          finish({status:200,data:{ok:true,best:bestLocal,score:pts,beacon:true}});
          return;
        }
      }catch(e){}
      const img=new Image();
      img.onload=()=>finish({status:200,data:{ok:true,best:bestLocal,score:pts,beacon:true}});
  ...
  async function markStart(){
    if(!HAS_API) return;
    try{ await apiPost('/api/snake/start',{}); }catch(e){
      try{
        const q='sessionId='+encodeURIComponent(SESSION_ID)+'&token='+encodeURIComponent(TOKEN);
        await fetch(API_BASE+'/api/snake/start?'+q,{method:'GET',cache:'no-store'});
      }catch(e2){}
    }
  }

  function applySaveResult(d, pts){
    if(d&&d.ok){
      if(typeof d.best==='number'&&d.best>bestLocal){
        bestLocal=d.best; bestEl.textContent=bestLocal;
      }
      setSave('ok', d.newRecord?'¡Nuevo récord!':'Guardado en el top',
        pts+' pts · récord '+(d.best!=null?d.best:bestLocal)+(d.total!=null?' · total '+d.total:''));
      return true;
    }
    const err=d&&d.error||'error';
    let msg='No se pudo guardar';
    if(err==='imposible') msg='Score rechazado (anti-trampa)';
    else if(err==='cooldown') msg='Espera '+(d.wait||'unos')+'s';
    else if(err==='sesion'||err==='expirada'||err==='token'||err==='auth') msg='Sesión expirada · vuelve a /snake';
    else if(err==='alto') msg='Máximo '+(d.max||5000)+' pts';
    setSave('err','No guardado', msg);
    return false;
  }

  async function submitScore(pts){
    if(!HAS_API){
      setSave('err','Sin servidor','Configura snakePublicUrl en el bot');
      return;
    }
    if(saving) return;
    saving=true;
    setSave('', 'Guardando…', pts+' puntos');
    try{
      let r=null;
      try{ r=await apiPost('/api/snake/score',{score:pts}); }catch(e){ r=null; }
      if(r&&r.data&&r.data.ok){ applySaveResult(r.data,pts); return; }
      try{ r=await apiGetScore(pts); }catch(e){ r=null; }
      if(r&&r.data&&r.data.ok){ applySaveResult(r.data,pts); return; }
      r=await apiBeaconScore(pts);
      if(r&&r.data&&r.data.ok){
        setSave('ok','Enviado al top', pts+' pts (si el servidor respondió)');
        return;
      }
      if(r&&r.data){ applySaveResult(r.data,pts); return; }
      const httpHint=API_BASE.indexOf('https://')!==0
        ? 'WhatsApp bloquea HTTP. Usa HTTPS (Cloudflare Tunnel / dominio).'
        : 'Revisa firewall del puerto en Ptero.';
      setSave('err','Sin conexión', httpHint);
    }catch(e){
      setSave('err','Sin conexión','WhatsApp bloqueó la red · usa HTTPS');
    }finally{
      saving=false;
    }
  }
  ...
  function reset(){
    ...
    dir=nextDir={x:1,y:0};
    food=randFood();
    score=0; alive=true; last=0; saving=false;
