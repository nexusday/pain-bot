...
export function obtenerNombreJugadorMiner(jid, conn, participants = null) {
  return limpiarNombre(getDisplayName(jid, conn, participants))
}

function svgAvatarPorDefecto(letter, color = '#78350f', size = AVATAR) {
...
async function obtenerBufferPerfil(conn, jid, name, size = AVATAR) {
  if (!conn || !jid) return sharp(Buffer.from(svgAvatarPorDefecto(name, '#78350f', size))).png().toBuffer()
  try {
  ...
  return sharp(Buffer.from(svgAvatarPorDefecto(name, '#78350f', size))).png().toBuffer()
}
...
function iconoTrofeo(cx, cy, sc = 1) {
  return `<g transform="translate(${cx},${cy}) scale(${sc})" filter="url(#glowF)">
...
function renderizarCeldaMiner(index, cell, highlight) {
  ...
  const icon = gain ? iconoGema(cx, cy, 0.85) : iconoMina(cx, cy, 0.85)
...
function panelJugador(x, name, score, active, color, champion = false) {
  ...
  return `
    ...
    <text x="${x + 100}" y="230" font-family="Arial,sans-serif" font-size="19" font-weight="700" fill="#fef3c7">${escaparXml(name)}</text>
...
export function construirSvgTableroMiner(game, options = {}) {
  const {
    status = 'playing',
    statusText = '',
    highlightIndex = null,
  ...
  const p1Name = obtenerNombreJugadorMiner(p1, conn, participants)
  const p2Name = obtenerNombreJugadorMiner(p2, conn, participants)
  ...
  const cells = (game.cells || []).map((cell, i) => renderizarCeldaMiner(i, cell, i === highlightIndex)).join('\n')

  let header = ''
  if (status === 'finished') {
    header = `
      <rect x="56" y="52" width="608" height="88" rx="22" fill="#1f1608" stroke="#fbbf24" stroke-width="2"/>
      ${iconoTrofeo(360, 34, 0.9)}
      ...
      <text x="360" y="118" text-anchor="middle" font-family="Arial,sans-serif" font-size="15" fill="#fde68a">${escaparXml(footer)}</text>`
  } else {
    header = `
      ${iconoPico(360, 56, 1.1)}
  ...
  ${header}
  ${panelJugador(56, p1Name, s1, status === 'playing' && turnP1, '#fbbf24', p1Win)}
  ${panelJugador(376, p2Name, s2, status === 'playing' && !turnP1, '#d97706', p2Win)}
  ${cells}
  <rect x="48" y="890" width="${WIDTH-96}" height="54" rx="16" fill="#120e08" stroke="#5c4a2a" stroke-width="1.5"/>
  <text x="360" y="923" text-anchor="middle" font-family="Arial,sans-serif" font-size="16" font-weight="700" fill="${footerColor}">${escaparXml(footer)}</text>
...
export function construirSvgInvitacionMiner({ challengerName, opponentName, moneda = 'USD' }) {
  ...
  <text x="184" y="390" text-anchor="middle" font-family="Arial,sans-serif" font-size="18" font-weight="700" fill="#fef3c7">${escaparXml(challengerName)}</text>
  ...
  <text x="536" y="390" text-anchor="middle" font-family="Arial,sans-serif" font-size="18" font-weight="700" fill="#fef3c7">${escaparXml(opponentName)}</text>
  ...
  <text x="648" y="488" text-anchor="end" font-family="Arial,sans-serif" font-size="15" font-weight="700" fill="#fbbf24">450 ${escaparXml(moneda)}</text>
...
export async function renderizarPngTableroMiner(game, conn, options = {}) {
  const participants = options.participants || null
  const svg = construirSvgTableroMiner(game, { ...options, conn, participants })
  let base = await svgAPng(svg, BOARD_H)
  const half = AVATAR / 2 + 3
  base = await componerAvatares(base, conn, [
    { jid: game.player1, name: obtenerNombreJugadorMiner(game.player1, conn, participants), left: 56 + 52 - half, top: 220 - half, size: AVATAR, color: '#fbbf24' },
    { jid: game.player2, name: obtenerNombreJugadorMiner(game.player2, conn, participants), left: 376 + 52 - half, top: 220 - half, size: AVATAR, color: '#d97706' },
  ])
  return base
}

export async function renderizarPngInvitacionMiner(conn, challenger, opponent, participants = null) {
  const cName = obtenerNombreJugadorMiner(challenger, conn, participants)
  const oName = obtenerNombreJugadorMiner(opponent, conn, participants)
  const svg = construirSvgInvitacionMiner({ challengerName: cName, opponentName: oName, moneda: global.moneda || 'USD' })
...
export async function enviarTableroMiner(conn, chat, game, opts = {}) {
  const png = await renderizarPngTableroMiner(game, conn, opts)
...
export async function enviarInvitacionMiner(conn, chat, opts = {}) {
  const png = await renderizarPngInvitacionMiner(conn, opts.challenger, opts.opponent, opts.participants)
  const mentions = (opts.mentionedJid || []).filter(Boolean)
...
export function obtenerGanadorMiner(game) {
...
export {
  obtenerNombreJugadorMiner as getMinerPlayerName,
  construirSvgTableroMiner as buildMinerBoardSvg,
  construirSvgInvitacionMiner as buildMinerInviteSvg,
  renderizarPngTableroMiner as renderMinerBoardPng,
  renderizarPngInvitacionMiner as renderMinerInvitePng,
  enviarTableroMiner as sendMinerBoard,
  enviarInvitacionMiner as sendMinerInvite,
  obtenerGanadorMiner as getMinerWinner
}
