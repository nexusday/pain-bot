...
import { aplicarQuotedVerificado } from './quoted-verificado.js'
...
export async function enviarViaLia(conn, jid, content, options = {}) {
  ...
  try {
    const esEdicion = !!(content && typeof content === 'object' && content.edit)
    const opciones = await aplicarQuotedVerificado(conn, options || {})
    ...
    const mensajeEditado = msg.message?.protocolMessage?.editedMessage
    const mensajeParaBiz = mensajeEditado
      ? (normalizeMessageContent(mensajeEditado) || obtenerMensajeInterno(mensajeEditado) || mensajeEditado)
      : mensajeInterno
    ...
    if (esEdicion) {
      opcionesRelay.additionalAttributes = {
        ...(opcionesRelay.additionalAttributes || {}),
        edit: '1',
      }
    }

    if (shouldIncludeBizBinaryNode(mensajeParaBiz)) {
      const nodoBiz = getBizBinaryNode(mensajeParaBiz)
