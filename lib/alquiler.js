...
export async function notificarSinPlan(m, conn) {
  ...
  if (!entry.noPlanNotified) {
    ...
    const botonSoporte = construirBotonSoporte(conn)
    const linkSoporte = obtenerLinkSoporte(conn)
    ...
    const text =
      ...
      `> ${p}rent infinito\n` +
      `para habilitarlo aquí sin límite de tiempo.\n\n` +
      (linkSoporte
        ? `*¿Quieres alquilar el bot?*\nEntrá al grupo de *soporte* con el botón.`
        : `*¿Quieres alquilar el bot?*\nPedile al dueño el link de *soporte*.`)

    try {
      if (botonSoporte && typeof conn.sendMessageLia === 'function') {
        await conn.sendMessageLia(m.chat, {
          text,
          footer: global.packname || global.namebot || 'Pain-Bot',
          nativeFlow: [botonSoporte],
...
export async function notificarAlquilerExpirado(m, conn) {
  ...
  if (!rental.notifiedExpired) {
    ...
    const text =
      ...
      (linkSoporte
        ? `Para renovar, entrá al grupo de *soporte* con el botón.`
        : `Para renovar, pedile al dueño el link de *soporte*.`)
