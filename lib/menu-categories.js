...
export const MENU_BUTTON_TEXT = '『 Ver Categorias 』'

const cmd = (p, name) => `> ・ ${p}${name}`
const sectionHeader = (name) => `ー ${name} ー`
const pickerRow = (name) => `・ ${name}`
const pickerGroup = (name) => `ー ${name} ー`
...
function sectionGamesRpg(p) {
  return `${sectionHeader('𝙶𝙰𝙼𝙴𝚂 𝚁𝙿𝙶')}
${cmd(p, 'balance')}
${cmd(p, 'bal')}
${cmd(p, 'coins')}
...
const CATEGORY_GROUPS = [
  ...
  { title: pickerGroup('𝚁𝙿𝙶'), ids: ['juegos', 'perfil', 'top'] },
...
const CATEGORY_DEFS = [
  ...
  { id: 'juegos', label: '𝙶𝙰𝙼𝙴𝚂 𝚁𝙿𝙶', hint: 'economía · gacha · casino', roles: ['all'], body: sectionGamesRpg },
