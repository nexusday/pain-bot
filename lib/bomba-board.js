...
export function obtenerNombreJugadorBomba(jid, conn, participants = null) {
  return limpiarNombre(getDisplayName(jid, conn, participants))
}

function svgAvatarPorDefecto(letter, color = '#7f1d1d', size = AVATAR) {
...
async function obtenerBufferPerfil(conn, jid, name, size = AVATAR) {
  if (!conn || !jid) return sharp(Buffer.from(svgAvatarPorDefecto(name, '#7f1d1d', size))).png().toBuffer()
  try {
  ...
  return sharp(Buffer.from(svgAvatarPorDefecto(name, '#7f1d1d', size))).png().toBuffer()
}
...
function iconoBomba(cx, cy, sc = 1) {
...
function iconoSeguro(cx, cy, sc = 1) {
  return `<g transform="translate(${cx},${cy}) scale(${sc})" filter="url(#glowF)">
...
function iconoCalavera(cx, cy, sc = 1) {
  return `<g transform="translate(${cx},${cy}) scale(${sc})" filter="url(#glowF)">
...
function renderizarCelda(index, cell, game, options = {}) {
...
function panelJugador(x, name, active, color, champion = false) {
  ...
  return `
    ...
    <text x="${x + 100}" y="230" font-family="Arial,sans-serif" font-size="19" font-weight="700" fill="#fecaca">${escaparXml(name)}</text>
...
export function construirSvgTableroBomba(game, options = {}) {
  const {
    status = 'playing',
    statusText = '',
    highlightIndex = null,
  ...
  const p1Name = obtenerNombreJugadorBomba(p1, conn, participants)
  const p2Name = obtenerNombreJugadorBomba(p2, conn, participants)
  ...
  const cells = (game.cells || []).map((cell, i) =>
    renderizarCelda(i, cell, game, { highlightIndex, status, revealAll })
  ).join('\n')
  ...
  if (status === 'finished') {
    header = `
      ...
      <text x="360" y="118" text-anchor="middle" font-family="Arial,sans-serif" font-size="15" fill="#fecaca">${escaparXml(footer)}</text>`
  } else {
    header = `
      ${iconoBomba(360, 56, 1.05)}
      ${iconoMecha(320, 40)}
  ...
  ${header}
  ${panelJugador(56, p1Name, status === 'playing' && turnP1, '#f87171', p1Win)}
  ${panelJugador(376, p2Name, status === 'playing' && !turnP1, '#fb923c', p2Win)}
  ...
  <text x="616" y="310" text-anchor="end" font-family="Arial,sans-serif" font-size="16" font-weight="800" fill="#fca5a5">${game.pot || game.bet * 2} ${escaparXml(moneda)}</text>
  ${cells}
  <rect x="48" y="900" width="${WIDTH-96}" height="54" rx="16" fill="#12080c" stroke="#5c2a2a" stroke-width="1.5"/>
  <text x="360" y="933" text-anchor="middle" font-family="Arial,sans-serif" font-size="16" font-weight="700" fill="${footerColor}">${escaparXml(footer)}</text>
...
export function construirSvgInvitacionBomba({ challengerName, opponentName, bet, moneda = 'USD' }) {
  const pot = bet * 2
  ...
  <text x="184" y="390" text-anchor="middle" font-family="Arial,sans-serif" font-size="18" font-weight="700" fill="#fecaca">${escaparXml(challengerName)}</text>
  ...
  <text x="536" y="390" text-anchor="middle" font-family="Arial,sans-serif" font-size="18" font-weight="700" fill="#fecaca">${escaparXml(opponentName)}</text>
  ...
  <text x="648" y="488" text-anchor="end" font-family="Arial,sans-serif" font-size="15" font-weight="700" fill="#fca5a5">${bet} ${escaparXml(moneda)}</text>
  ...
  <text x="648" y="542" text-anchor="end" font-family="Arial,sans-serif" font-size="15" font-weight="700" fill="#86efac">${pot} ${escaparXml(moneda)}</text>
  ...
  <text x="648" y="596" text-anchor="end" font-family="Arial,sans-serif" font-size="15" font-weight="700" fill="#fca5a5">${bet} ${escaparXml(moneda)}</text>
...
async function svgAPng(svg, height = BOARD_H) {
  try {
...
async function componerAvatares(base, conn, slots) {
  if (!conn || !slots?.length) return base
  const composites = []
  for (const { jid, name, left, top, size, color } of slots) {
    const raw = await obtenerBufferPerfil(conn, jid, name, size)
    const av = await construirAvatarCircular(raw, size, color, 3)
...
export async function renderizarPngTableroBomba(game, conn, options = {}) {
  const participants = options.participants || null
  const svg = construirSvgTableroBomba(game, { ...options, conn, participants })
  let base = await svgAPng(svg, BOARD_H)
  const half = AVATAR / 2 + 3
  base = await componerAvatares(base, conn, [
    { jid: game.player1, name: obtenerNombreJugadorBomba(game.player1, conn, participants), left: 56 + 52 - half, top: 220 - half, size: AVATAR, color: '#f87171' },
    { jid: game.player2, name: obtenerNombreJugadorBomba(game.player2, conn, participants), left: 376 + 52 - half, top: 220 - half, size: AVATAR, color: '#fb923c' },
  ])
  return base
}

export async function renderizarPngInvitacionBomba(conn, challenger, opponent, bet, participants = null) {
  const cName = obtenerNombreJugadorBomba(challenger, conn, participants)
  const oName = obtenerNombreJugadorBomba(opponent, conn, participants)
  const svg = construirSvgInvitacionBomba({ challengerName: cName, opponentName: oName, bet, moneda: global.moneda || 'USD' })
  let base = await svgAPng(svg, INVITE_H)
...
function construirSeccionesCeldas(game) {
...
function construirCamposInteractivos(sections, turnName) {
  return {
...
export async function enviarTableroBomba(conn, chat, game, opts = {}) {
  const png = await renderizarPngTableroBomba(game, conn, opts)
  ...
  if (interactive && isInteractiveBaileysEnabled()) {
    const turnName = obtenerNombreJugadorBomba(game.currentPlayer, conn, opts.participants)
    const sections = construirSeccionesCeldas(game)
    if (sections.length) {
      try {
        const content = {
          image: png,
          caption,
          ...construirCamposInteractivos(sections, turnName),
          contextInfo: ctx,
        }
...
export async function enviarInvitacionBomba(conn, chat, opts = {}) {
  const png = await renderizarPngInvitacionBomba(conn, opts.challenger, opts.opponent, opts.bet, opts.participants)
  const mentions = (opts.mentionedJid || []).filter(Boolean)
...
export {
  obtenerNombreJugadorBomba as getBombaPlayerName,
  construirSvgTableroBomba as buildBombaBoardSvg,
  construirSvgInvitacionBomba as buildBombaInviteSvg,
  renderizarPngTableroBomba as renderBombaBoardPng,
  renderizarPngInvitacionBomba as renderBombaInvitePng,
  enviarTableroBomba as sendBombaBoard,
  enviarInvitacionBomba as sendBombaInvite
}
