          ...
          case constants.TYPE_LOSSY:
            {
              ...
              size += c.raw.length + 8;
              imgData = crearChunkBasico("VP8 ", c.raw);
            }
            break;
          ...
          case constants.TYPE_LOSSLESS:
            {
              ...
              imgData = crearChunkBasico("VP8L", c.raw);
            }
            break;
          ...
          case constants.TYPE_EXTENDED:
            if (img.data.extended.hasAnim) {
              ...
              if (img.data.vp8) {
                ...
                if (img.data.alph) {
                  alpha = true;
                  imgData.push(...crearChunkBasico("ALPH", img.data.alph.raw));
                  size += img.data.alph.raw.length + 8;
                }

                imgData.push(...crearChunkBasico("VP8 ", img.data.vp8.raw));
                size += img.data.vp8.raw.length + 8;
              } else if (img.data.vp8l) {
                imgData = crearChunkBasico("VP8L", img.data.vp8l.raw);
...
var _demuxFrame2 = /*#__PURE__*/ (function () {
  var _demuxFrame3 = _asyncToGenerator(function* (path, frame) {
    ...
    if (
      ...
      if (this.data.extended.hasICC) {
        out.push(...crearChunkBasico("ICCP", this.data.extended.icc.raw));
      }
    }

    if (frame.vp8l) {
      out.push(...crearChunkBasico("VP8L", frame.vp8l.raw));
    } else if (frame.vp8) {
      if (frame.vp8.alpha) {
        out.push(...crearChunkBasico("ALPH", frame.alph.raw));
      }

      out.push(...crearChunkBasico("VP8 ", frame.vp8.raw));
    } else {
    ...
    if (this.type == constants.TYPE_EXTENDED) {
      if (this.data.extended.hasEXIF) {
        out.push(...crearChunkBasico("EXIF", this.data.extended.exif.raw));
      }

      if (this.data.extended.hasXMP) {
        out.push(...crearChunkBasico("XMP ", this.data.extended.xmp.raw));
      }
    }
...
var _readChunk_VP2 = /*#__PURE__*/ (function () {
  var _readChunk_VP3 = _asyncToGenerator(function* (fd, size) {
    ...
    return {
      raw: buf,
      width: anchoVP8(buf, 8),
      height: altoVP8(buf, 8)
    };
  });
