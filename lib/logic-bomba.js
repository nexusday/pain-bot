...
async function bloquearApuestas(game) {
...
export async function aceptarInvitacion(m, conn, invite, participants = []) {
  try {
    ...
    if (!await bloquearApuestas(game)) {
      return conn.sendMessage(m.chat, {
    ...
    game.onTimeout = async () => {
      try {
        ...
        await manejarFinJuego(simulatedM, conn, game, groupParts, null, { forcedEnd: true })
      } catch (e) {
...
export async function manejarMovimiento(m, conn, gameData, participants = []) {
  try {
    ...
    if (!game.gameActive || res.hit || res.forcedEnd) {
      return manejarFinJuego(m, conn, game, groupParts, cellIndex, res)
    }
...
export async function manejarFinJuego(m, conn, game, participants = [], highlightIndex = null, lastRes = null) {
  try {
